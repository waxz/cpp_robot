...
//        DdsSimpleWriter(int history_depth, const std::shared_ptr<DdsSimpleParticipant>& paticipant, const char* topic_name , const char* topic_profile , const char* pub_profile, const char * writer_profile);
        DdsSimpleWriter(const std::shared_ptr<DdsSimpleParticipant>& paticipant, const WriterConfig& config);
...
//            participant_->participant_->delete_publisher(publisher_);
    ...
    template<typename T, typename S>
    DdsSimpleWriter<T,S>::DdsSimpleWriter(const std::shared_ptr<DdsSimpleParticipant>& paticipant, const WriterConfig& config)
            : participant_(paticipant), type_(new S()), config_(config)
    {

//        const char* pub_profile = config_.pub_profile.c_str();
//        const char* topic_profile = config_.topic_profile.c_str();
//        const char* topic_name = config_.topic_name.c_str();
//        const char* writer_profile = config_.writer_profile.c_str();
        ...
        paticipant->participant_->get_topic_qos_from_profile(config_.topic_profile, topic_qos);
...
//        DdsSimpleReader( ta_cfg_t* cfg  , int history_depth ,const std::shared_ptr<DdsSimpleParticipant>& participant, const char* topic_name , const char* topic_profile , const char* sub_profile, const char * reader_profile);
        DdsSimpleReader(ta_cfg_t* cfg  ,const std::shared_ptr<DdsSimpleParticipant>& participant, const ReaderConfig& config  );
        ~DdsSimpleReader();
    ...
    template<typename T, typename S>
    DdsSimpleReader<T,S>::DdsSimpleReader(ta_cfg_t* cfg ,const std::shared_ptr<DdsSimpleParticipant>& participant, const ReaderConfig& config)
            : participant_(participant), type_(new S()), config_(config)
            {

//
//                const char* sub_profile = config_.sub_profile.c_str();
//
//                const char* topic_profile = config_.topic_profile.c_str();
//                const char* topic_name = config_.topic_name.c_str();
//                const char* reader_profile = config_.reader_profile.c_str();
                int history_depth = config_.qos_queue_size;
        ...
        topic_ = participant_->participant_->create_topic(
                config_.topic_name,
                type_.get_type_name(),
                topic_qos);
        if (topic_ == nullptr)
...
#if 0
    template<typename T, typename S>
    DdsSimpleReader<T,S>::DdsSimpleReader(ta_cfg_t* cfg, int history_depth ,const std::shared_ptr<DdsSimpleParticipant>& participant, const char* topic_name, const char* topic_profile  , const char* sub_profile, const char * reader_profile)
            : participant_(participant), type_(new S())
    {
        ...
        //CREATE THE SUBSCRIBER
        eprosima::fastdds::dds::SubscriberQos sub_qos = eprosima::fastdds::dds::SUBSCRIBER_QOS_DEFAULT;
        participant_->participant_->get_subscriber_qos_from_profile(sub_profile, sub_qos);
        subscriber_ =  participant_->participant_->create_subscriber(sub_qos);
        if (subscriber_ == nullptr)
        {
            throw std::runtime_error("create_subscriber failed");
        ...
        //CREATE THE TOPIC
        eprosima::fastdds::dds::TopicQos topic_qos = eprosima::fastdds::dds::TOPIC_QOS_DEFAULT;
        participant_->participant_->get_topic_qos_from_profile(topic_profile, topic_qos);
    ...
    ChannelBuffer_ptr DdsSimpleReader<T, S>::read_data() {
        ...
        if (reader_->wait_for_unread_message(timeout)){
            if (ReturnCode_t::RETCODE_OK == reader_->take(dataseq_, infoseq_,2,eprosima::fastdds::dds::NOT_READ_SAMPLE_STATE)){
